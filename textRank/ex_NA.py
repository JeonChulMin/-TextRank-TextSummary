...
url = 'http://news.naver.com/main/read.nhn?mode=LSD&mid=shm&sid1=100&oid=421&aid=0003417256' # 크롤링할 url 주소 입력
...
text = """
(브뤼셀=연합뉴스) 김병수 특파원 = 마르크 뤼테 네덜란드 총리가 자신이 실수로 쏟은 커피를 청소부를 시키지 않고 막대 걸레로 직접 닦아 '결자해지'하는 모습이 인터넷을 통해 널리 퍼지며 전 세계적으로 화제가 되고 있다.

지난 5일 유튜브에 업로드된 동영상을 보면 뤼테 총리는 이날 커피를 종이컵에 담아 들고 헤이그에 있는 건물의 보안대를 통과하다가 실수로 커피를 바닥에 쏟았다.

뤼테 총리는 멋쩍은 웃음을 지어 보인 뒤 청소부에게 이를 처리하도록 내버려두지 않고 곧바로 들고 있던 서류를 보안대에 올려놓고는 옆에 있던 청소부에게 막대 걸레를 건네달라고 해서 바닥에 쏟아진 커피를 닦았다. 

뤼테 총리는 막대 걸레로 제대로 닦이지 않은 일부 커피는 손걸레로 훔치기도 했다.

막대 걸레질을 하는 뤼테 총리의 모습을 발견한 건물 내 다른 청소부 여러 명이 박수를 치며 뤼테 총리에게 다가왔고, 한 청소부는 키가 큰 뤼테 총리가 막대 걸레의 길이가 짧아서 허리를 굽히고 걸레질을 하자 총리에게 다가와 막대 길이를 늘여주며 '청소 노하우'를 전수하기도 했다. 

뤼테 총리는 함박웃음을 보이며 열심히 청소를 마무리했다. 

이 영상은 네덜란드 공영방송인 NOS 방송 카메라에 포착돼 제일 처음 아랍권의 미디어에서 방영된 뒤 곧바로 독일 등 유럽 내 여러 나라와 미국, 인도 등에서 널리 유포되고 있다고 네덜란드의 NOS 방송은 전했다.
...
미국의 워싱턴포스트는 뤼테 총리의 모습에 대해 '심볼 오브 에티켓'이라고 극찬했다.
"""
