...
URL = 'http://news.naver.com/main/read.nhn?mode=LSD&mid=shm&sid1=100&oid=020&aid=0003151320'
OUTPUT_FILE_NAME = 'output.txt'
# 위는 해당 URL 주소로 요청을 보내고 받아 기사 내용을 파싱해 하나의 문자열로 저장하는 함수
def get_text(URL):
    source_code_from_URL = urllib.request.urlopen(URL)
    # urlopen 메소드를 통해 URL 주소에 대한 요청을 보내 source_code_from_URL 변수에 그 결과를 저장
    soup = BeautifulSoup(source_code_from_URL, 'lxml', from_encoding='utf-8')
    # HTML 코드를 파상하기 위해 source_code_from_URL을 이용해서 BeautifulSoup 객체를 생성해 soup에 할당
    # 기존 html 방식 대신 lxml 방식으로 파싱, 한글 기사이기 때문에 utf-8 방식으로 인코딩

    # 기사 본문 내용 div 클래스안에 담겨져 있음
    # 따라서 우리가 생성한 객체 soup에서 위에서의 div클래스를 가져오기만 하면 됩니다.
    text = ''
    ...
    return text
    # 이후 find_all 메소드를 이용해 id가 'articleBodyContents'인 div 클래스를 모두 뽑아내고
    # for문을 통해 뽑힌 요소 하나하나에 다시 한번 직접 접근
    # for문으로 뽑혀진 각 요소 item에 find_all메소드를 사용, text 키워드 인자에 True를 넣어 텍스트 요소만 뽑아 문자열로 치환한 후 text를 반환

    # 본문 내용에 포함된 요소가 div클래스 하나인데 for문을 쓴 이유는 첫 번째로는 find_all 메소드의 반환 객체가 인자로 주어진 조건을 만족하는
    # 모든 요소들을 순환가능한 객체로 주어지기 때문에 사용
    # 두 번째로는 다른 신문사의 경우, 본문 내용이 동일한 id를 갖는 동일한 여러 개의 태그에 나눠져 할당되어 있는 경우도 있기 때문
...
class Sentence:
    ...
    def __init__(self, text, index=0):
        ...
        self.all_pos = [ad for ad in twitter.pos(self.text, norm = True, stem = True) if ad not in _stopwords]
        self.adjectives = []
        for i, n in self.all_pos:
            if n == 'adjective':
                self.nouns.append(i)
